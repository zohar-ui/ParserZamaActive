 ...
 * Updated: January 10, 2026 - Use pg client for zamm schema
 * ============================================
 */
...
const CONFIG = {
  ...
  dbHost: process.env.SUPABASE_DB_HOST || 'db.dtzcamerxuonoeujrgsu.supabase.co',
...
async function initDatabase() {
  if (!CONFIG.dbPassword) {
    console.error('❌ ERROR: SUPABASE_DB_PASSWORD not found in environment');
...
async function fetchUntrainedExamples() {
  ...
  const query = `
    ...
    WHERE is_included_in_training = false
      AND learning_priority >= $1
    ORDER BY learning_priority DESC, created_at DESC
    LIMIT $2
  `;
  
  try {
    const result = await client.query(query, [CONFIG.minPriority, CONFIG.maxNewExamples]);
    console.log(`   Found ${result.rows.length} examples (priority >= ${CONFIG.minPriority})`);
    return result.rows;
  } catch (error) {
...
async function markExamplesAsTrained(examples) {
  ...
  const query = `
    ...
    SET is_included_in_training = true,
        included_in_training_at = NOW()
    WHERE example_id = ANY($1)
  `;
  
  try {
    await client.query(query, [exampleIds]);
